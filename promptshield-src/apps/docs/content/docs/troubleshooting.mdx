...
Check PromptShield is pointed at Bifrost and configure provider credentials in
Bifrost:
...
Run `curl -s http://localhost/health` to confirm the product gateway is up.
...
- Confirm the provider key and route are configured in Bifrost
