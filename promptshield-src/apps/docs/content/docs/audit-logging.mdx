...
PromptShield writes audit events to stdout. In the production Compose product,
the `log-fwd` sidecar tails the gateway container and posts events to the
dashboard ingest endpoint.

```bash
docker compose logs -f promptshield-gateway
```

The [Dashboard](/docs/dashboard) provides the searchable audit view.
