...
description: Built-in exporter on /metrics for internal Prometheus scraping.
---

Uses `prometheus/client_golang`. No flag to enable it. Prometheus scrapes the
gateway on the internal Compose network.
...
scrape_configs:
  - job_name: promptshield
    static_configs:
      - targets: ["promptshield-gateway:8080"]
    scrape_interval: 15s
```
